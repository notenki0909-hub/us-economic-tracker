...
- **次回発表予定日**：FREDの各系列ページが持つ「Next Release Date」メタデータから取得し、
  ...
  公式データ源が構造的に存在しないため「なし」と表示**（詳細は下記）。
...
一方、**市場予想（コンセンサス）は実装していない**。これは政府統計機関ではなく民間データベンダー
（Bloomberg・Reuters等）がエコノミストへの調査を集計する商用データであり、無償・登録不要で
配信するソースが構造的に存在しない：
...
`marketConsensus` フィールドは全指標で `null` 固定とし、フロントエンドで「なし（無償で配信する
公式データ源が存在しないため掲載していません）」と表示している。将来、無償ソースが見つかった場合や、
有償APIキーの利用を許容する方針転換があった場合に備え、スキーマ上は最初から用意してある。
